...
function Stats({ dark }) {
  ...
  const { total, completadas, pendientes, vencidas, progress } = useMemo(() => {
    ...
    return { total, completadas, pendientes, vencidas, progress }
  }, [tasks])

  const values = { total, completadas, pendientes, vencidas }

  return (
    <div>
      <div className="grid grid-cols-4 gap-3 mb-6">
        {STATS_CONFIG.map(s => (
          <div key={s.label} className="rounded-2xl px-5 py-4 border dark:bg-[#13131f] bg-white dark:border-[#1e1e30] border-[#e0dff0]">
            <div className="text-[11px] font-medium uppercase tracking-wide opacity-50 mb-2">
              {s.label}
            </div>
            <div className={`text-2xl font-bold leading-none ${s.color}`} style={{ fontFamily: "DM Sans, sans-serif" }}>
              {values[s.key]}
            </div>
          </div>
        ))}
